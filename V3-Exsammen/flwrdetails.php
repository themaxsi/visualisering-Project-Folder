<?php 
    ...
    {
        $result = $db->query("SELECT * FROM flowers WHERE FID = $ID");
        
    }
    $productRow = $result->fetch_assoc();
    for ($i=0; $i < 4; $i++) 
    { 
        if (isset($productRow["FImg"])) //checks if the variable has a value
        {
            if (strpos($productRow['FImg'], " "))  //if there is more then one
            {
                $img = explode(" ", $productRow["FImg"]);
            }
            else //if there is 1 pic
            {
                $img[0] = $productRow['FImg'];
            }
        }
    ...
    <main class="arrdetails">
        <section>
            <article>
                ...
                ?>
                <p class="arrprice">Price from <?php echo $productRow['FPrice'] ?></p>
            </article>
